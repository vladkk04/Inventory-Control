...
class PdfReportGenerator(private val context: Context) {
    ...
    companion object {
        private const val PDF_PAGE_WIDTH = 595 // A4 width in points (1pt = 1/72 inch)
        private const val PDF_PAGE_HEIGHT = 842 // A4 height
    ...
    fun generateStockReport(
        ...
        creationDate: String,
        periodReports: TimePeriodReports, // Added time period parameter
        ...
        var currentY = 50f // Initial margin
        ...
        // Date and Period
        paint.textSize = 12f
        canvas.drawText("Date: $creationDate", 50f, currentY, paint)
        canvas.drawText("Period: ${periodReports.name.replace("_", " ")}",
            PDF_PAGE_WIDTH - 250f, currentY, paint)
        currentY += 40f
        ...
        if (products.isEmpty()) {
            ...
            canvas.drawText("No stock data available for the selected period",
                50f, currentY, paint)
            currentY += 30f
        } else {
            ...
            products.forEach { product ->
                ...
                // Create new page if we reach the bottom
                if (currentY > PDF_PAGE_HEIGHT - 50) {
                    doc.finishPage(page)
                    val newPageInfo = PdfDocument.PageInfo.Builder(PDF_PAGE_WIDTH, PDF_PAGE_HEIGHT, doc.pages.size + 1).create()
